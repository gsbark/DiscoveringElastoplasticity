...
from Utils.material_laws import ST_update_NN,ST_update_NN_hardening
...
class MPM_program_inv:
    def __init__(self,MPM_config,Material,Analysis_config,Geometry,device,epochs):
        ...
        self.wp_scale_out = wp.array([scale_y],dtype=wp.float32,requires_grad=True,device=device)
    ...
    def step_NN(self,step,m_param):
        ...
        if self.isHardening:
            wp.launch(kernel=Get_NN_input_3d,
                  dim=[self.num_particles],
                  inputs=[self.States.stress[step],self.States.gamma[step],self.States.theta[step],
        ...
        self.run_NN(self.mlp_inp1[step],self.wp_mlp.out1[step],self.wp_mlp.out2[step],
                    self.wp_mlp.out3[step],self.wp_mlp.f_val[step],step,get_jacobian=True)
        self.run_NN(self.mlp_inp2[step],self.wp_mlp.out11[step],self.wp_mlp.out22[step],
                    self.wp_mlp.out33[step],self.wp_mlp.f_max[step],step,get_jacobian=False)
        
        if self.isHardening:
            wp.launch(
                kernel=self.Stress_update_hardening,
                dim=[self.num_particles],
                inputs=[self.MPM_var,self.States.stress[step],self.States.log_e[step],self.States.gamma[step],self.States.L[step],
                        self.wp_mlp.f_val[step],self.wp_mlp.f_max[step],self.wp_mlp.df_dx_true[step],
                        self.States.stress[step+1],self.States.log_e[step+1],self.States.gamma[step+1],m_param]
                        )
        else:
            wp.launch(
    ...
    def Stress_update(MPM:MPM_vars,
        ...
        tid = wp.tid()
        tau,e = ST_update_NN(L[tid],log_e[tid],stress[tid],
    ...
    def Stress_update_hardening(MPM:MPM_vars,
        ...
        tid = wp.tid()
        tau,e,new_pl = ST_update_NN_hardening(L[tid],log_e[tid],stress[tid],e_pl[tid],
                                              f_val[tid],f_max[tid],dF_dinp[tid,0],dF_dinp[tid,1],dF_dinp[tid,2],
                                              MPM.mu,MPM.lamda,MPM.dt,m_param)
        new_stress[tid] = tau
        new_log_e[tid] = e
        new_e_pl[tid] = new_pl
